"""
PhiEx.adapters — every external tool / ML model wears the same coat.
...
Architectural rule (the most important one in this repo):
    ...
    tool's API and `PhiEx.core.state` + `PhiEx.core.calculator`
    + `PhiEx.core.learned`.  Tool-specific types (Bio.PDB.Structure,
...
PHIEX_STRICT=1 is set; otherwise they fall through to a clearly-marked
